...
void Grid::slotDataChanged(void)
{
    ...
    if (mimeData->hasText())
    {
        ...
        for(int i=0;i<(int)(reslist.count());i+=3)
        {
           ...
           emit ui->leTdn->editingFinished();
           QApplication::processEvents();
           ui->leM->setText(reslist[i+2]);
           ui->leM->setModified(true);
           emit ui->leM->editingFinished();
           QApplication::processEvents();
        }
    }
}
...
void Grid::ShowTable(void)
{
    ...
    {
        dtable = "";
        for(int i=0; i<table.length(); i+= 40)
        {
            dtable += QStringView{table}.mid(i,40);
            dtable += "\n";
        }
        table = dtable;
    }
