...
void ARBchannel::Show(void)
{
    ...
    leSWFREQ->installEventFilter(this);
    leSWFREQ->setMouseTracking(true);
    leSWFVRNG->installEventFilter(this);
    leSWFVRNG->setMouseTracking(true);
    leSWFVAUX->installEventFilter(this);
    leSWFVAUX->setMouseTracking(true);
    leSWFVOFF->installEventFilter(this);
    leSWFVOFF->setMouseTracking(true);
}

bool ARBchannel::eventFilter(QObject *obj, QEvent *event)
{
    if(moveWidget(obj, gbARB, gbARB , event)) return true;
    if (((obj != leSWFREQ) && (obj != leSWFVRNG) && (obj != leSWFVAUX) && (obj != leSWFVOFF))) return QObject::eventFilter(obj, event);
...
bool ARBchannel::SetValues(QString strVals)
{
    ...
    if(resList[5] == "FWD")
    {
        ...
        emit SWFDIR_FWD->clicked(true);
    }
    else if(resList[5] == "REV")
    {
        SWFDIR_REV->setChecked(true);
        emit SWFDIR_REV->clicked(true);
    }
