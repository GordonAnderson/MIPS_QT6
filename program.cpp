...
void Program::setBootloaderBootBit(void)
{
    ...
    msgBox.exec();
    // Set boot bit
    cmd = appPath + "/bossac -b --port=" + comms->serial->portName() + " -R";
...
void Program::saveMIPSfirmware(void)
{
    ...
    cmd = appPath + "/bossac -r -b --port=" + comms->serial->portName() + " " + fileName + " -R";
    executeProgrammerCommand(cmd);
}
...
void Program::programMIPS(void)
{
    ...
    msgBox.exec();
    // Program MIPS
    cmd = appPath + "/bossac -e -w -v -b --port=" + comms->serial->portName() + " " + fileName + " -R";
    executeProgrammerCommand(cmd);
}
...
void Program::programRFmega(void)
{
    ...
    msg += " Make sure you have a new RFmega binary file to load, it should have a .bin extension.\n ";
    msg += "The RFmega firmware will be erased so if your bin file is invalid or fails to program, RFmega will be rendered useless!\n";
    msgBox.setText(msg);
    msgBox.setInformativeText("Are you sure you want to contine?");
    msgBox.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
    msgBox.setDefaultButton(QMessageBox::Yes);
    ...
    pui->tabMIPS->setCurrentIndex(1);
    // Select the binary file we are going to load to MIPS
    QString fileName = QFileDialog::getOpenFileName(this, tr("Load RFmega firmware .bin file"),"",tr("Files (*.bin *.*)"));
    if(fileName == "") return;
    // Program RFmega
    cmd = appPath + "/bossac -e -w -v -b --offset=0x4000 --port=" + comms->serial->portName() + " " + fileName + " -R";
    executeProgrammerCommand(cmd);
}
