    ...
    for(i = 0;i<P.size();i++)
    {
        ...
        else
        {
            ...
            for(j = 0; j < 16; j++)
            {
                ...
                {
                    QString textV;
                    textV = QString::asprintf("%.3f", P[i]->DCbias[j]);
            ...
            for(j = 0; j < 16; j++)
            {
                if(P[i]->DigitalO[j] != P[i - 1]->DigitalO[j])
                {
                    ...
                    if (P[i]->DigitalO[j]) sTable += ":" + QString(((QChar)'A').unicode() += j) + ":1";
                    else sTable += ":" + QString(((QChar)'A').unicode() += j) + ":0";
                    NoChange = false;
                }
            }
            // If nothing changed then update DO A, something has to be defined at this time point
...
void PSG::on_pbViewTable_pressed()
{
    ...
    {
        dtable = "";
        for(int i=0; i<table.length(); i+= 60)
        {
            dtable += QStringView{table}.mid(i,60);
            dtable += "\n";
        }
        table = dtable;
    }
