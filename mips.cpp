...
//
// Note!
//  Opening the serial port on the native USB connection to the DUE and setting the
...
//          4, 5, and 6 bit
//      2.) Added plot capabilities to support heat maps, for the FAIMSFB work
// 1.55, July 13, 2019
...
//      3.) Added app signing
// 1.90, Oct 12, 2021
//      1.) Fixed a bug in the scripting system that did not do a full string compare
...
// 1.94, March 1, 2022
//      1.) Minor bug fix on timing generation reading and writting the time mode checkbox
// 1.95, March 11, 2022
//      1.) Fixed PlotCommand bug when x axis steps are not integers
//      2.) Updated event control to ignore ? return from table read update
// 1.96, June 22, 2022
//      1.) Fixed RFChannel control SetValue function, the Setpoint and Freq were
//          reversed.
// 1.97, July 25, 2022
//      1.) Added the text message option to the contol panel function
// 1.98, Aug 7, 2022
//      1.) Added tcpSocket client support to control panel code.
// 1.99, Oct 2, 2022
//      1.) Fix a bug in the plot function AddPoint routing. The system would crash if you
//          sent too many values to plot.
//      2.) Updated the programming function to include the port address
//      3.) Added a program RFmega function, will work for any itsybitsy M4
// 1.100, Nov 3, 2022
//      1.) Added SendSerial function to the exposed functions in mips.
//      2.) Added take focus to script buttons to improve UI
//      3.) Minor bug fixes
// 1.101, Nov 19, 2022
//      1.) Fixed a bug in the plotting function when multiple scans were added to a plot
// 1.102, Dec 23, 2022
//      1.) Added modbus support, specfically for Novus temp controllers
// 1.103, Dec 29, 2022
//      1.) fixed a bug that caused long time time calculations, actually it was an algorithm
//          improvement.
//      2.) fixed a bug the ARB compressor trigger from the table.
// 1.104, April 29, 2023
//      1.) Added STRINGL to custom control to support long strings
// 1.105, May 30, 2023
//      1.) Updates to the plotting function
// 1.106, Sept 19, 2023
//      1.) Added arrow up/down value adjust to custom controls
//      2.) Added the STEP command to config file to set up/down arrow
//          adjust size. This value applies to all Ccontrols that follow
//          the command.
//      3.) Added the ability to change the system text font size.
// 1.107, Nov 11, 2023
//      1.) Added Define to control paanel
//      2.) Added subroutines to control panel
//      3.) The control panel now allow a empty string to be returned from SendMessage
//      4.) Control panel now allows command to press control panel buttons
//      5.) Remove white space around token in control panel config file
// 1.108, Nov 20, 2023
//      1.) Added support for RF driver control from pulse sequence generator in the
//          control panel.
// 1.109, Mar 1, 2024
//      1.) Fixed a bug in the DCbias channel grouping function, will only work for
//          up and down arrow changing prior to this fix. It has been broken for a while!
// 1.110, April 14, 2024
//      1.) Added delete option to the plot commands
//      2.) Added GenerateUniqueFileName command to the scripting functions
// in process, June 10, 2024
//      1.) Fixed bug in control combobox, slot was depricated and I missed it in upgrade!
//          ComboBox control were not working.
//
// 2.0, June 23, 2024
//      1.) Completed port to QT 6.7.1
//      2.) Removed most warning, a few are left. One more depreicated area to rewirte, the
//          modbus code
//      3.) Added RemoveLast to the plot command function to allow removal of the last
//          generated plot, used in DMSDMS error recovery
//      4.) Found a number of bugs due to porting, this version should be stable
// 2.1beta,
//      1.)  Added mouse wheel value changing. Refactored all the up down arrow value changing
//      2.)  Allow saving comments with settings, if comment is present popup the comment
//           after loading settings.
//      3.)  Allow moving controls on panel, left click in top left corner to move.
//      4.)  Update cfg file with new position data after moves
//      5.)  Allow starting app with config file name and load, also make file assocation work.
//      6.)  Allow image loading on current object, ImageBox
//      7.)  Removed all the static control panel options from menu and remove code.
//      8.)  Removed IFT support
//      9.)  Renamed Select... to Load configuration...
//      10.) Removed the shutter from control panel
//      11.) Added embedding a plot on a form and allow scripts to access its data
//              a.) Added plot command to stop popup menu, NoPopup,TRUE or FALSE
//      12.) Set the the process events flag to false in the control panel code. It was true
//           and caused scripting issues due to the scrips running in a different thread.
//      Aug 28, 2024
//      13.) Fixed bug that prevented UpdateHalted flag being set to false when script aborts
//      14.) Added script button command to Abort.
//      15.) Fixed heatmap plotting bug, when averaging the heatmap scale was
//           set to the sum not the average
//      16.) Cleaned up the plot deleting, the resources were not freed properly
//      17.) Sept 22, Reduced serial received signal delay from 10 ms to 2ms
//      18.) Sept 27, Added plotonly command to the list of plot commands
//      19.) Sept 27, Added a number of additional plot commands and features
//          - OpenFile, caused a file save dialog to select a file to save data
//          - ClearComments, clears all comments in a plot file
//      20.) Sept 27, fixed lost focus change signal when selecting a checkbox, long standing bug
//      21.) Oct 1, 2024, fixed slow loading of plot file due to message object creation when not needed!
// 2.2, December 21, 2024
//      1.) Removed beta, this version is fully tested
//      2.) Added support of reading the option file from MIPS to change the
//          control panel
//      3.) Fixed a bug in the control panel call subroutine function
// 2.3, January 4, 2025
//      1.) Changed command for reading options from SD card to READSD,
//          it was READ interfered with another MIPS command
//      2.) Added support for ScriptButton call with parameters, the following
//          is not supported:
//          ScriptButton,bname,scriptname,"call(123,acb)",X,Y
//          The call must be in double quotes.
//          Script eaxmple:
//          Script,scriptname
//          (
//            function(a,b)
...
//          )
//          EndScript
//      3.) Fixed the slow find MIPS performancer by removing the bluetooth
//          search, it was very slow on close
//      4.) Improved the MIPS connection and now report the serial port
//          as well as removing all unused tabs
//      5.) Fixed the long standing bug with the terminal window vertical size
//          and cutting off half of the last line.
// 2.4, Feburary 2, 2025
//      1.) Added file IO function to scripting functions
// 2.5, Feburary 23, 2025
//      1.) Added the Common script to the control. This script is appened to all
//          scripts that are defined in a control panel
//      2.) Disallow MIPS comm button from all child control panels
//      3.) Added function sysUpdating to involkable functions, it always uses
//          the parent control panel
//      4.) Modified UpdateHalted and WaitForUpdate to use the parent control panel
// 2.6, March 13, 2025
//      1.) Fixed bug with RFamp, it was not displaying in a container.
//      2.) Added ProcessCommand function to RFamp allow access to its parameters.
//      3.) Fixed controls disappearing from control panel
//      4.) Fixed memory leak in plot
//      5.) Fixed the ESC key from unloading the control panel, maybe I should ask?
//      6.) Clear the MIPS infor area when the selected MIPS is changed
// 2.7, April 13, 2025
//      1.) Redesiged the script processing code. This update improves the stability
//          and now the thread and engine are created only one time and reused. This
//          removed a lot of complexity and should result in more stable performance.
//      2.) Added the scriptObj option to control panel and created a new class.
//          - Updated the Ccontrol class to allow attached a script to events on Ccontrols
//            This uses script commands to attache a striptObj to a Ccontrol event
//      3.) Studied the comms class and tired using loop and signals to improve the
//          wait for line, this did not work. The comms class does need a redesign at
//          some point.
//      4.) After a lot of testing the random crashing was due to the serial wait for line
//          function, the function was updated to remove any process event calls. This fixed
//          the app crashing issue. Also, stopped doing a serial port reconnect on
//          wait for line timeout error.
//      5.) Added color options to custom controls, DC bias channels controls, and RF driver
//          controls. This uses the script command function and the .color extension.
//      6.) Fixed a bug in the save control panel function
//      7.) Added options to read and write the scripts in script buttons and in
//          script objects
//      8.) Added involkable function to read en entire text file into a QString
//      9.) Updated arrow key values change to use the proper multiplier keys
//      10.)Disabled adjust values on Ccontrol STRING type
//      11.)Started working on setpoint readback update happening at a reduced rate
//           - Implemented Ccontrol
//           - Implemented DCBchannel
//           - Need to add this to additional controls
//           - Added additional feature
//              - Initial startup first update call performes at update
//              - Added SkipCount to config file commands, can be changed
//              - in the config load process for different controls
// 2.8, July 17, 2025
//      1.) Removed mutex from comms and added log message is comms is re-entered
// 2.9, July 22, 2025
//      1.) Added Table to control panel, this allows displaying a table of values
// 2.10, August 14, 2025
//      1.) Removed most of the process events calls from the control panel code.
//          This should improve the performance of the control panel and reduce
//          the number of issues with the scripting engine.
//      2.) Added a state machine to the control panel update class to prevent
//          the long running update function from blocking the control panel.
//      3.) Added channel and mips commands to the DCbias channel class.
//      4.) Added script to run on table cell change.
//      5.) Added color and hide to group boxes.
//      6.) Remove process updates from most of comms, the delay in comms now
//          issues a sleep.
//      7.) Added script command to custom controls to allow setting and reading
//          the mips commands and setting the tool tip
//      8.) Added script command to custom controls to allow setting the color
//          of the control.
//      9.) Added serial commands to support async serial port processing.
//      10.) Updated the PlotCommands
//          - Added the addpoints command to support CSV list of values
//          - Addressed stability issues
//          - Added Y axis auto scale
//      11.) Added a scrolling groupbox option to the control panel.
//      12.) Fixed the tab control naming and nesting in the control panel. You can now
//           have tab control inside a tab control or any other control like a groupbox.
// 2.11, August 22, 2025
//      1.) Changed control panel update to update ccontrols in steps of 10 at a time.
//          This should improve the UI response when running a control panel.
//      2.) Fx several minor bugs.
// 2.12, Sep 5, 2025
//      1.) Added script with arguments to custom control evelts and Table events.
//      2.) fixed bug that caused a crash is custom control object was not properly defined
//          and the script system tried to write to the control
// 2.13, Sep 14, 2025
//      1.) Added abort script to the script button command list, this script is executed
//          when the script is aborted.
// 2.14, Sept 27, 2025
//      1.) Added slider control to control panels
// 2.15, Oct 5, 2025
//      1.) Fixed a bug in the vertical version of the slider control
//      2.) Fixed but in slider values being loaded from the settings file
//      3.) Added .WidthT to the timing generator's set of commands
//      4.) Fixed a bug in the timing generator that caused the system to crash if trigger was pressed
//          when not connected to MIPS.
// 2.16, Oct 17, 2025
//      1.) Added load plot option to file menu
// 2.17, Oct 25, 2025
//      1.) Added setValue and getValue functions to scripting invokable functions
//      2.) Wrote a python program to process MIPS firmware command and build MIPScommands.txt
//          help file. Updated the MIPScommand.txt file
//      3.) Added lasso type zooming to plot function, to enable uncomment LassoMode define
//          in plot.cpp file.
// 2.18, Nov 22, 2025
...
// Todo list
//      1.) Update help
//          - MIPS commands (can this be automatically generated?)
//          - Control panel generation and commands
//          - Script commands
//      5.)  Fix naming issue on timing generator
//      6.)  Check nesting, something does not work, I think its tabs in a tab
//      10.) Automatic control panel generation and save
//      13.) Allow defining a control's color
//      14.) Support readback error turing red/green in custom controls
//      15.) Dynamic update rates, slow down the settings readback some how.
//           Could read the set values only on startup and every nth update,
//           make n a large number and random
//           use: int randomNumber = generator.bounded(1, 5);
//      16.) Add script call event on custom control event
//      17.) Set custom control default
//      18.) Redesign comms to run in a seperate thread, this will prevent UI blocking
//           on timeouts and waiting for response.
//
#include "mips.h"
#include "ui_mips.h"
#include "console.h"
...
*/
    ...
    connect(ui->actionLoad_Plot, SIGNAL(triggered()), this, SLOT(loadPlot()));
...
void MIPS::DisplayProperties(void)
{
    ...
    //QFont _Font("Tahoma", properties->sysFontSize.toInt());
 ...
 * \param newPC
 *
 * This slot is called when the control panel dialog is closed.
 * If a new control panel file name is provided then that control panel is opened.
 * If no file name is provided then the control panel pointer is set to NULL and
 * the main window is re-enabled.
 */
 ...
 * \param fileName
 *
 * This function opens a control panel dialog for the MIPS system defined in the provided file name.
 * If a control panel is already open then this function exits.
 * The main window is disabled while the control panel is open.
 */
